...
-- 권한 관리
-- mdguest, mdguest2 사용자에게 
-- 데이터베이스 접속(connect) 및 테이블 생성(resource) 권한, 테이블스페이스를
-- 무제한으로 사용할 수 있는 권한(unlimited tablespace)을 부여하시오.
...
-- 컬럼: 사용 권한을 부여할 테이블의 열 이름을 지정
-- 객체: 사용 권한을 부여할 객체를 지정. 테이블이나 뷰 등을 말함
-- to 사용자: 사용자나 권한의 묶음인 롤(role)에 권한을 추가 가능
-- public은 모든 사용자가 사용할 수 있도록 공개적으로 권한을 부여한다는 의미
-- with grant option: 허가받은 권한을 다른 사용자에게 다시 부여 가능
