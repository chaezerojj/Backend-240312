...
-- customer테이블을 orders테이블과 조건없이 연결
select * from customer, orders;
-- 고객5명 * 주문10건 = 총 50개의 행 출력됨
select * from customer;
select * from orders;
-- 예) 박지성의 custid가 1이므로 orders테이블에서 custid가 1인것과 연결
...
-- 조인을 사용하면 2개 이상의 테이블에서 일부 데이터만 얻을 수 있음
...
-- 3개 이상의 테이블을 조인
select * from orders;
-- 마당서점에 주문된 내역을 고객 정보와 책 정보까지 모두 보이시오.
select *
...
-- 오라클에서는 +기호로 외부조인을 수행할 수 있음
-- 왼쪽 외부조인인 경우 (+)기호를 조인조건의 오른쪽에
-- 오른쪽 외부조인인 경우 (+)기호를 왼쪽에 붙임
